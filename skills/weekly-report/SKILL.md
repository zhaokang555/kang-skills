...
- **日记仓库**：当前 git 仓库根目录（`git rev-parse --show-toplevel`）
...
运行 `git rev-parse --show-toplevel`，取输出作为 `<journalDir>`。
...
检查 `<日记仓库>/<YYYY>/<MM>/<YYYY-MM-DD>-weekly-report.md` 是否已存在，若存在则输出"已存在，跳过"并停止。
...
```bash
mkdir -p <日记仓库>/<YYYY>/<MM>
```

将生成内容（末尾加换行）写入 `<日记仓库>/<YYYY>/<MM>/<YYYY-MM-DD>-weekly-report.md`。

### 7. 提交

```bash
git -C <日记仓库> add <YYYY>/<MM>/<YYYY-MM-DD>-weekly-report.md
git -C <日记仓库> commit -m "weekly-report: <YYYY-MM-DD>"
```
